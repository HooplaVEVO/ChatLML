...
static void host_loop(ChatBox& chatbox, const std::string& username) {
    ...
    if (new_socket < 0) {
        chatbox.addMessage(username, "Chatroom creation failed");
        return;
    }
    chatbox.addMessage(username, "Chatroom successfully initialized!");
    ...
    while (true) {
        ...
        if (rec != temp) {
            rec = temp;
            chatbox.addMessage(username, rec);
        }
...
static void join_loop(ChatBox chatbox,std::string username,std::string IP){
    ...
    if(client.init(IP)!=0){ //Unsuccessful server initialization
        chatbox.addMessage(username, "Chatroom join failed");
        return;
    }
    else{
        chatbox.addMessage(username, "Chatroom successfully initialized!");
